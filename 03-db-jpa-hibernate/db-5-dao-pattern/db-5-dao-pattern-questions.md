...
| 질문 | 답변 요약 |
| --- | --- |
| 문자열 대신 enum을 쓰면 뭐가 좋은데? | enum은 미리 정해진 상수 집합을 정의할 수 있는 특별한 클래스야. `"PAID"` 같은 문자열을 직접 쓰면 오타 위험이 있는데, `OrderStatus.PAID`처럼 enum을 쓰면 컴파일 타임에 오류를 잡을 수 있어. 안전하고 자동완성도 돼서 실무에서 자주 사용돼! |

---

### 💡 8. enum을 DB에 저장할 땐 어떻게 해?

| 질문 | 답변 요약 |
| --- | --- |
| enum을 그대로 pstmt에 set할 수 있어? | DB에는 문자열로 저장해야 하므로 `enum.name()`을 사용해서 `setString()`에 넘겨야 해. 예: `pstmt.setString(4, order.getStatus().name());` |

---

### 💡 9. 그럼 SELECT할 때는 어떻게 다시 enum으로 바꿔?

| 질문 | 답변 요약 |
| --- | --- |
| 문자열로 꺼낸 DB 값을 어떻게 다시 enum으로 바꿔? | `OrderStatus.valueOf(rs.getString("status"))`를 사용하면 문자열을 다시 enum으로 바꿀 수 있어. 단, DB에 저장된 값과 enum 이름이 정확히 일치해야 해! |

---

### 💡 10. status 같은 필드, enum 말고도 방법이 있어?

| 질문 | 답변 요약 |
| --- | --- |
| enum 대신 문자열 검증도 가능해? | 가능해! `Set<String>`이나 `List<String>`을 만들어두고, 사용자가 입력한 값이 그 안에 있는지 `contains()`로 체크하는 방법도 있어. enum을 쓰면 더 안전하지만, 간단한 프로젝트에서는 Set으로도 충분해. |
...
- enum → 상수 집합으로 오타 방지, 컴파일 시 안정성 확보
- enum.name(), valueOf() → DB ↔ enum 변환 핵심
- Set.contains → 간단 검증 대안